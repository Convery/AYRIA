...
namespace Hash
{
    namespace Tigerdata
    {
        constexpr std::array<uint64_t, 256> Tiger1 =
        {
        ...
        constexpr std::array<uint64_t, 256> Tiger2 =
        {
        ...
        constexpr std::array<uint64_t, 256> Tiger3 =
        {
        ...
        constexpr std::array<uint64_t, 256> Tiger4 =
        {
    ...
    namespace Tigerinternal
    {
        using namespace Tigerdata;
        ...
        struct State_t
        {
            uint64_t A{ uint64_t(0x0123456789abcdefLL) };
            uint64_t B{ uint64_t(0xfedcba9876543210LL) };
            uint64_t C{ uint64_t(0xf096a5b4c3b2e187LL) };
    ...
    // Any range.
    template <cmp::Range_t T> constexpr std::array<uint8_t, 24> Tiger192(const T &Input)
    {
        Tigerinternal::State_t State{};
        Tigerinternal::Write(State, std::span<const typename T::value_type>(Input));
        return Tigerinternal::Finalize(State);
    }

    // Any other typed value.
    template <typename T> requires (!cmp::Range_t<T>) constexpr std::array<uint8_t, 24> Tiger192(const T &Input)
    {
        return Tiger192(cmp::getBytes(Input));
    }

    // String literals.
    template <cmp::Char_t T, size_t N> constexpr std::array<uint8_t, 24> Tiger192(const T(&Input)[N])
    {
...
namespace Unittests
{
    static_assert("61d26192cf832c07612c541552d80027bbb3f520064f48ec" == String::toHex(Hash::Tiger192("12345")), "BROKEN: Tiger192 hashing");
}
#endif
